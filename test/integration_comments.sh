...
# These assertions run against al-folio's demo posts. This site does not ship
# them, so skip rather than fail when they are absent.
if [ ! -f "${giscus_page}" ] && [ ! -f "${disqus_page}" ]; then
  echo "comments integration checks skipped (no demo comment posts in this site)"
  exit 0
fi
